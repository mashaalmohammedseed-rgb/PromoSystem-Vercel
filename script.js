...
if (referralLinkInput) {
    // Note: Replit will use its own URL, which is fine for the referral link
...
function loadData(){
  ...
  invitedFriends.forEach(u=>{
    // Note: 'document' now holds the document type
    html+=`<tr>
      <td>${u.name}</td>
      <td>${u.country}</td>
      <td>${u.document || '-'}</td>
...
form.addEventListener('submit', async e=>{
  ...
  const payload={
    ...
    phone: phone,
    country: country,
    document: documentType, // Sending the document type now
  ...
  if (!name || !country || !documentType) {
      status.className = 'msg error';
      status.textContent = 'الاسم والدولة ونوع المستند مطلوبان.';
      return;
  }

  // Simulate saving the invited friend to the inviter's local storage
  // This part is for demonstration only, a real system would use a database
  if (inviterId) {
